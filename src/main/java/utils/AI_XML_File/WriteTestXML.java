...
public class WriteTestXML {
    ...
    public void getInfoFromJson(String jsonFilePath) throws Exception {
        AI_Parser parser = new AI_Parser(jsonFilePath);
        setTestSuiteName(parser.getTestSuiteName());
...
//                + "_" + Utility.getTime()
    ...
    public void createFolders(String workingDirectory) throws Exception {

        // create folder to store test xml file under working directory
        setTestSuiteFolderName(workingDirectory);
        Utility.createFolder(workingDirectory + File.separator + "testSuite");
        Utility.createFolder(getTestSuiteFolderName());

        // set a path of output testng.xml file (will rename as ai.xml)
        setTestNgXmlPath(workingDirectory + File.separator + "ai.xml");

        // create folder to store test class .java file under src/test/java
        setTestClassFolderName( workingDirectory + "/src/test/java/" + getTestSuiteName() );
        Utility.createFolder(getTestClassFolderName());
    ...
    public void createXml(String workingDirectory, String jsonFilePath) throws Exception {
        getInfoFromJson(jsonFilePath);
        createFolders(workingDirectory);
        addDocType();
        this.doc.appendChild(addSuiteRootElement());
    }
...
//        StreamResult result = new StreamResult(new File(getTestSuiteFolderName() + "/ai.xml"));
    ...
    public void writeToFile(String filePath) throws TransformerException, IOException {
        StringWriter outputXmlStringWriter = new StringWriter();
...
//        StreamResult result = new StreamResult(new File(getTestSuiteFolderName() + "/ai.xml"));
        ...
        String xmlFile = getTestSuiteFolderName() + File.separator + "ai.xml";
        FileOutputStream outputXml = new FileOutputStream(new File(xmlFile));
        outputXml.write(outputXmlString.getBytes("UTF-8"));

        System.out.println("Write content to " + xmlFile + " succesfully!");
    }
